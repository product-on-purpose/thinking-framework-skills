...
The `META_SKILLS` exemption is narrow, and deliberately so: it covers the checks that ask "does this skill have a registry entry", not every check in the file. The lifecycle-truth section scopes itself to skill *directories* precisely so tools are included - they ship just as hard as frameworks, and before that section existed they were the easiest place for a stale lifecycle field to hide. A tool that is shipped but has never been through the behavioral eval says so honestly: `status: active` with `maturity: alpha`, which is the state all four tools are in today.
