...
5. **Recipe-command drift** (`scripts/gen-recipe-commands.mjs --check`) - each of the nine recipes ships a generated `commands/think-<recipe>.md` that carries the chain verbatim; this layer regenerates them and fails on any byte of drift, so the command a user invokes cannot quietly diverge from the recipe it is named for.
