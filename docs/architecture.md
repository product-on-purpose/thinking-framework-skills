...
1. **Structural** - the `agent-skills-toolkit` validators (`evaluate.mjs`), pinned to a known-good ref, asserting the plugin meets the `advanced` tier with 0 errors. It currently carries 128 non-gating warnings: 121 post-0.8 documentation rules held at `warn` by the declared-version ceiling, and 7 `U5` description-quality findings on the contested lenses. See `docs/conformance.md`.
