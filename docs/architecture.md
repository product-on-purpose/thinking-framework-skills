...
1. **Structural** - the `agent-skills-toolkit` validators (`evaluate.mjs`), pinned to a known-good ref, asserting the plugin meets the `advanced` tier with 0 errors. It currently carries 7 `U5` description-quality warnings, all on the contested lenses, whose descriptions lead with an evidence caveat rather than a trigger phrase; see `docs/conformance.md`.
