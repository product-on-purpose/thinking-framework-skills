...
The `META_SKILLS` exemption is narrow, and deliberately so: it covers the checks that ask "does this skill have a registry entry", not every check in the file. The lifecycle-truth section scopes itself to skill *directories* precisely so tools are included - they ship just as hard as frameworks, and before that section existed they were the easiest place for a stale lifecycle field to hide. A tool that is shipped but has never been through the behavioral eval says so honestly: `status: active` with `maturity: alpha`. Since 2026-09-10 three of the four tools (`think-framework-advisor`, `think-top3`, `think-random-frameworks`) have been through both halves and read `maturity: measured`; `think-research-framework` remains `alpha` because its output half is not measurable by the generic produce-then-judge harness - it dispatches to a subagent with web search rather than emitting an artifact inline.
...
`scripts/check.mjs` is the single required gate (a status check on `main`); CI runs it on every PR. It runs fifteen layers in order, and any failure is a red build:

1. **Structural** - the `agent-skills-toolkit` validators (`evaluate.mjs`), pinned to a known-good ref, asserting the plugin meets the `advanced` tier with 0 errors. It also carries a three-figure count of non-gating warnings: post-0.8 documentation rules held at `warn` by the declared-version ceiling, plus seven `U5` description-quality findings on the contested lenses. `docs/conformance.md` states the current number, its breakdown, and its provenance - deliberately in one place, because a count copied onto three pages goes stale on two of them.
...
3. **Registry** (`scripts/check-registry.mjs`) - schema validation, generated-view drift, referential integrity (shipped <-> skill dir, fold targets resolve to a shipped skill, dossier paths and source URLs resolve), completeness, the IP/attribution lint, eval-coupling, tier consistency, the registry <-> advisor recommendable cross-check, lifecycle-metadata truth (every skill directory on disk carries `identity.status: active`, and a skill whose two `quality.*_eval_status` stamps both read `measured-*` carries `maturity: measured`), and the workflows mirror (every `_workflows/*.md` is declared in `library.json` `components.workflows` and every declared workflow exists on disk). Both rule sets live in pure libs (`scripts/lib/lifecycle-lib.mjs`, `scripts/lib/workflow-mirror-lib.mjs`) so they are unit-tested directly rather than only exercised against the real tree.

   The workflows mirror deliberately duplicates a check the toolkit also makes (ADR 0047, `S3`). The toolkit's version is capped at `warn` until a consumer pins Standard 0.15, and this repo pins 0.8, so without the local copy nine undeclared recipes would have gone on shipping invisibly for however long the pin holds.
4. **Engine drift** (`scripts/gen-engine.mjs --check`) - the shared applicator engine copy is in sync.
5. **Recipe-command drift** (`scripts/gen-recipe-commands.mjs --check`) - each of the nine recipes ships a generated `commands/think-<recipe>.md` that carries the chain verbatim, and the two allowlisted recipes also ship a generated subagent (`agents/think-<recipe>.md`) that preloads exactly the chain's skills; this layer regenerates both and fails on any byte of drift, or on a generated subagent whose recipe left the allowlist, so neither surface a user invokes can quietly diverge from the recipe it is named for.
6. **AGENTS.md drift** (`scripts/gen-agents.mjs --check`) - the generated Skills + Recipes tables in the contributor/agent guide are in sync with the catalog, so the agent-facing roster cannot silently fall behind.
...
10. **Contested-lens contract** (`scripts/check-contested.mjs`) - every contested lens (a `caveatFirst` registry entry) leads with its evidence caveat across `SKILL.md` / `TEMPLATE` / `EXAMPLE` / sample / eval-cases per its posture (`run_caveat_first` or `warn_redirect`), a branded lens carries its trademark attribution on every surface, and the marker agrees across the registry, the SKILL.md frontmatter, and the `skill.meta.yml` sidecar. The deterministic core is `scripts/lib/contested-lib.mjs` (unit-tested, both postures negative-tested).
11. **Mermaid validity** (`scripts/check-mermaid.mjs`) - every mermaid block in repo docs and committed site content is syntactically valid, so a broken diagram cannot silently ship.
12. **Canonical links** (`scripts/check-canonical-links.mjs`) - every internal link in the repo docs resolves without redirect hops, so stale paths surface immediately.
13. **Repo-markdown links** (`scripts/check-repo-links.mjs`) - every relative link in repo-facing markdown resolves to a file or anchor that actually exists.
14. **Changelog consistency** (`scripts/check-changelog.mjs`) - `CHANGELOG.md` and `RELEASE-NOTES.md` agree on the most recent version, so a version bump cannot update one file and forget the other, and no version block repeats a Keep a Changelog change type, so a run of PRs each prepending its own `### Added` block cannot accumulate unnoticed.
