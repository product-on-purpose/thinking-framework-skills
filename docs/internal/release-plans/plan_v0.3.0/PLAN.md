...
### Registry follow-ups (deferred from the SP3 adversarial review)

The SP3 build closed the *drift* loop on the evidence grade (check-registry now verifies each shipped entry's governing `tier` is one of the grades in its `SKILL.md` evidence-tier). Three deeper design questions are deferred:

1. **User-facing tier representation across surfaces.** The generated `framework-catalog.md` shows the single governing grade (e.g. `M`); the advisor's `recommendable.json` and the site framework pages show the skill's compound grade (e.g. `M/P`). These are now CI-verified *consistent* (governing grade is a token of the compound grade), but the displayed *strings* differ by surface. Decide whether to unify (catalog shows the compound grade too, or all surfaces collapse to the governing grade) - a presentation call, not a correctness one.
2. **Full single-sourcing of advisor/site metadata from the registry.** Today `recommendable.{json,md}` + the site pages derive tier/family/description from each `SKILL.md` (the registry cross-checks shipped-slug parity + tier consistency). The stronger move is to *generate* those surfaces from the registry so there is literally one source. Larger; ties to SP4 (the site Framework Library) and would change `recommendable.json` bytes (losing the compound grade) unless the registry stores it. Keep the cross-check until then.
3. **Family-taxonomy harmonization.** Registry `family` uses the 11-family catalog universe taxonomy; skill `metadata.family` uses a coarser live taxonomy (e.g. `reasoning-clarity`, `synthesis`) that the shipped skills + Astro `families/` pages use, and the two are not 1:1 (skill `reasoning-clarity` spans catalog families 4 + 9). There is deliberately no registry<->skill family consistency check. Decide whether to unify the taxonomies or keep them separate with a documented mapping.
