...
- **OPEN, security: `site/` has regressed to 10 vulnerabilities including a critical.** Re-measured 2026-09-10 at the v0.14.0 cut: `npm audit` in `site/` reports 10 (2 low, 3 moderate, 4 high, 1 critical) against an unchanged `package-lock.json`, so these are new advisories on the same pinned versions - notably a **critical Astro XSS** (GHSA-26w7-cxv4-gfx2, unescaped spread attribute names in `renderHTMLElement`), plus high findings on `js-yaml`, `sharp`, `smol-toml` and `svgo`. `npm audit fix` reports a non-breaking fix is available for part of it; the rest still needs the major bumps below. Deliberately NOT taken inside the v0.14.0 cut, because a release stamps a version on the code CI validated rather than changing dependencies underneath it. **This should be the next PR after the cut.** Build-time only: the exposure is a developer machine and CI, not a plugin consumer.
