...
- **OPEN, and now diagnosed: C2-7 / ADV-1, the front door is unmeasured because the instrument cannot measure it.** `think-framework-advisor`, `think-top3`, `think-random-frameworks` and `think-research-framework` all report `trigger_eval_status: authored`. This is not a missing run; it is a missing instrument.
  ...
  - **What to build:** a sibling `skill-selection.workflow.mjs` routing against the **`name` + `description` pairs an agent actually sees** (the 67 skills plus the 10 commands), rather than against `recommendable.json`. That instrument would (a) give the four meta-skills their first real trigger baseline, closing ADV-1; (b) be the correct instrument for **guardrail 6**, which the current one cannot satisfy - see the note below; and (c) make the advisor-rewrite proposal in the 2026-07-10 audit decidable on evidence, since it argues for a description change and there is currently no way to score one.
- **Note on guardrail 6, worth writing down before it is relied on again.** The guardrail requires a footprint delta *and* a routing re-run for every new command, recipe or subagent. The re-run was done for v0.14.0 and reproduced cleanly, but it **cannot detect what the guardrail is worried about**: the nine recipe commands added always-loaded *description* surface, while the trigger eval routes against `recommendable.json`, which the commands never touched (verified: unchanged since 2026-06-19). So the re-run is a genuine reproducibility check and a genuine confirmation that catalog routing is intact - but it is not evidence that the added surface did not interfere with skill selection, because it does not look there. The skill-selection instrument above is what would make guardrail 6 mean what it says.
