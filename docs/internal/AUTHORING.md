...
7. **Fill `skill.meta.yml`** (id, family, relationships, failure modes) from the dossier.
   ...
   **Lifecycle fields, and the one rule the gate enforces.** `identity.status` is the *skill's* lifecycle and `identity.maturity` is its *measurement* state; do not conflate them, and do not use either to describe how settled the sidecar schema is (that belongs in the header comment). Scaffold at `status: draft` / `maturity: alpha`, promote to `status: active` when the skill ships, and to `maturity: measured` once both `quality.*_eval_status` fields read `measured-*`. A skill stays `alpha` until BOTH halves are measured. The meta-skills are the worked example: since 2026-09-10 three read `maturity: measured` (trigger by the skill-selection eval, output by the meta-skills output eval), while `think-research-framework` stays `alpha` on a measured trigger and an unmeasured output half - a shipped skill is not entitled to `measured` because most of its evidence is in. Gate layer 3 enforces both rules (`scripts/lib/lifecycle-lib.mjs`), so a shipped skill cannot ship calling itself a draft.
...
9. **Validate to zero errors at the conformance gate:**
   ```
   node scripts/check.mjs        # advanced tier at Standard 0.8, 0 errors (a three-figure count of non-gating warnings is expected; docs/conformance.md has the number)
   npm test                      # the node --test suites
   ```
   The gate resolves the `agent-skills-toolkit` at the CI-pinned ref (clone it next to this repo, or as a `.agent-skills-toolkit` worktree). Require `advanced` and `0 error(s)` across every layer. Do **not** require zero warnings: post-0.8 documentation rules are held at `warn` by the declared-version ceiling and the seven caveat-first lenses each carry a `U5`, so a green run is a three-figure warning count. Errors gate; warnings do not. For a site-affecting change also run `npm --prefix site run build` and the link/route guards (`scripts/check-rendered-links.mjs`, `scripts/check-route-parity.mjs`). Fix anything flagged.
