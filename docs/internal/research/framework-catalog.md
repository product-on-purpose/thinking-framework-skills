...
| C-K theory (Concept-Knowledge design theory) | partition design work into a concept space of undecidable propositions and a knowledge space, and iterate the four operators (C-C, C-K, K-C, K-K) between them | C | `[excl]` |
...
| Estimate-talk-estimate (Delphi) | collect independent estimates, share only the anonymized spread and rationales, discuss the variance, then re-estimate independently | P | `[excl]` |
...
| Framework | Mechanism | Tier | Status |
|---|---|---|---|
| Trait-lens perspective (Big Five / HEXACO) | view a decision through contrasting personality-trait viewpoints (high vs low openness, conscientiousness, and so on) | C | `[fold]` -> Parallel Perspectives Review |
| VIA character-strengths lens | frame a problem or role through signature strengths drawn from the 24-strength classification | C | `[fold]` -> Parallel Perspectives Review |
| Kirton Adaption-Innovation (cognitive style) | read a problem, plan, or team through the adaptor (do it better) and innovator (do it differently) style poles | C | `[fold]` -> Parallel Perspectives Review |
| RIASEC interest-fit (Holland codes) | score a role, option, or person against the six fixed interest types (realistic, investigative, artistic, social, enterprising, conventional) and read the fit | P | `[fold]` -> Decision Option Review |
| Strong Interest Inventory | profile vocational interests against occupational reference groups | M | `[flag]` (branded) |
| Belbin Team Roles | map a team against nine complementary contribution roles to surface gaps and overlaps | C | `[excl]` (branded) |
| Tuckman group-development stages | diagnose a team's stage on the forming-storming-norming-performing arc to pick the fitting intervention | P | `[excl]` |
| Myers-Briggs Type Indicator (MBTI) | sort a person into one of sixteen types across four dichotomies | X | `[excl]` (branded) |
| CliftonStrengths (StrengthsFinder) | rank a person across 34 talent themes from a proprietary forced-choice assessment | V | `[excl]` (branded) |
| DISC profile | classify behavioral style across dominance, influence, steadiness, conscientiousness | V | `[excl]` (branded) |
| Enneagram of Personality | map a person to one of nine interconnected types with wings and stress / growth arrows | C | `[excl]` |
| Learning-styles inventories | match instruction to a learner's preferred modality (visual, auditory, kinesthetic, and so on) | X | `[excl]` (branded) |
