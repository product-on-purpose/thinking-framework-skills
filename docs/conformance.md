...
5. **Recipe-command drift** (`scripts/gen-recipe-commands.mjs --check`) - every `commands/think-<recipe>.md`, and each allowlisted recipe's `agents/think-<recipe>.md` subagent, is regenerated byte-identically from its `_workflows/` source, so a hand-edited recipe command or subagent cannot drift from the chain it claims to run.
