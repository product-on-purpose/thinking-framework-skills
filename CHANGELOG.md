...
### Changed
- **IP policy applied (open the IP gate, keep the evidence gate).** Every branded/trademarked method (Six Thinking Hats, Cynefin, Wardley Mapping, Blue Ocean, Disney Creative Strategy, Kepner-Tregoe, ICE/RICE/WSJF, Porter's Five Forces) now carries attribution + a trademark/license string and is re-tagged on evidence + distinctness alone - documented, not shipped. The evidence gate is unchanged: X/V/C grading and the ~20% overlap ceiling still keep SWOT, Five Whys, and role-played dissent out on the merits. [SP9]
- `site/.../about/why-not.md` is now a generated index of the deliberately-not-shipped set (folded / documented-not-shipped / excluded-on-the-merits / out-of-scope), each fold linking to the shipped skill it folds into - replacing the hand-curated essay with a view that cannot drift from the registry. [SP9]
- `docs/internal/research/framework-catalog.md` is now a generated view of the registry (family tables between `<!-- generated -->` markers; the taxonomy/legend/priority preamble and the exclusions/sources postamble stay hand-authored). [SP3]
