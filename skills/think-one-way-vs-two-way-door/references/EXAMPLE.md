...
A completed run of the `think-one-way-vs-two-way-door` skill on a real decision. This is the quality bar a generated classification should meet.
...
- **Next tool (one-way doors only):** `think-decision-option-review` to compare free tier vs the alternatives against weighted criteria, then `think-premortem` to stress-test the chosen path before commit.
