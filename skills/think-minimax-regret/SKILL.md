...
4. **Build the payoff matrix, and state where the figures came from.** For each option-by-state cell, record the payoff of that option in that state (profit, net value, utility - state the unit and the sign convention). If the situation supplies the figures, use them. If it does not, do NOT fabricate precise values and present them as given: mark each cell as an explicit estimate (e.g. `8 (assumed)`) and make the pick conditional on those assumptions, or flag a genuinely unknown cell (`?`) and say the matrix cannot resolve until it is supplied. Add a one-line payoff-provenance note. Invented numbers dressed as real inputs are the exact failure this step exists to prevent.
