...
(Sweep note for maintainers: this method sits beside a possible future `three-horizons` candidate - also foresight, also vertical/temporal layering - and the two should be vetted against each other before both are built, since a single "lightweight foresight layering" skill could collapse them. `three-horizons` is not currently a shipped skill and is therefore not a routing target.)
