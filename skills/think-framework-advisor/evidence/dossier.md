...
> The single source of truth for the `think-framework-advisor` skill. The `SKILL.md`, the
...
**First BLIND measurement (2026-09-10), and the first not self-authored in its scoring.** Every figure above comes from the advisor's own 12 cases, scored against the advisor's own catalog - small-N and self-referential, which the paragraph above says. The **skill-selection eval** (`scripts/eval/skill-selection.workflow.mjs`) asks a different and harder question: given the **installed roster** of 67 skills plus 10 commands, reduced to the name and description an agent actually sees, does a blind router reach for `think-framework-advisor` at all? Over the four entry-point tools' 17 trigger cases: **16 top-1 (94.1%), zero false fires**, one gate case excluded by construction. The single miss is one advisor case that reads to a router as a decision to pressure-test rather than a routing request. Across the whole 794-case corpus the run returned 391/393 top-1 and 399/400 no-false-fire - **not** a clean sweep; the one false fire belongs to `think-pairwise-comparison`, not to the advisor. Scorecard: `docs/internal/eval-results/2026-09-10-skill-selection-trigger-eval.{md,json}`, on `claude-opus-5`.

**The grade still does not rise, and the reason has changed.** Until 2026-09-10 routing was `C` because it was measured only against itself. It stays `C` now for a narrower reason: the blind run is genuinely blind and genuinely external to the advisor's own corpus, but it is one run, on one model, over 17 self-authored trigger cases, and it measures *selection of the tool* rather than *quality of the routing decision the tool then makes*. The second of those is the output eval's question, and it was answered separately (**10/10** on 2026-09-11 under a judge instructed to verify the artifact's claims about other skills against their own files). Two measurements of two different things, neither of which makes the routing rule itself validated.
