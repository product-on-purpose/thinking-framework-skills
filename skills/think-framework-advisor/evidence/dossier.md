...
2. **The routing accuracy is now first-measured, not validated** (section 3b). This is a stronger caveat than the usual transferred-evidence flag and is specific to a meta/router skill: a 2026-06-03 self-hosted eval gives a stable name-safety floor (12/12) but only a soft, run-to-run-variable routing signal (7/12 then 9/12 after test recalibration) on small-N, self-authored cases, so *whether this particular decision table reliably selects the right method* is measured but not validated. The skill mitigates this structurally - it shows its diagnosis and `Source:` citations so a user can challenge the routing ("why this job, why this framework?"), and it always lists what it chose **not** to recommend.
